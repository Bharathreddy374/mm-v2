...
console.log("Using ElevenLabs TTS:", !!process.env.ELEVENLABS_API_KEY);
...
const ELEVEN_VOICE_ID = process.env.ELEVEN_VOICE_ID || "21m00Tcm4TlvDq8ikWAM"; // default voice id — replace as needed
...
async function synthesizeWithElevenLabs(text) {
  ...
  const body = {
    text,
    model: "eleven_multilingual_v1", // or whichever model you prefer
    voice_settings: {
      stability: 0.5,
  ...
  try {
    ...
    return audioBuffer.toString("base64"); // base64 MP3
  } catch (err) {
    // bubble up useful error info
    const message = err.response && err.response.data
      ? `ElevenLabs error: ${JSON.stringify(err.response.data)}`
      : err.message;
...
`;
    ...
    // ---------- ElevenLabs TTS ----------
    const audioBase64 = await synthesizeWithElevenLabs(openingLine);
...
`;
    ...
    // 8. TTS for AI question via ElevenLabs
    const audioBase64 = await synthesizeWithElevenLabs(aiResponse);
...
-------------------------
1) ElevenLabs:
   - Pros: very natural voices, easy HTTP API; returns audio bytes.
   - Cons: paid, API key required. Set ELEVENLABS_API_KEY and optionally ELEVEN_VOICE_ID.

2) AssemblyAI TTS alternative:
   - If you already use AssemblyAI, they also support TTS. If you prefer that, I can swap to AssemblyAI TTS instead.
   - AssemblyAI TTS uses similar HTTP calls; let me know and I will replace the helper.

3) AWS Polly:
   - If you want an infra-free option (no third-party paid voice service) consider AWS Polly via aws-sdk (needs AWS credentials).
   - I can provide a version that uses Polly if you already have AWS creds.

If you want, I can:
- Add caching of generated audio (to avoid repeated TTS calls for identical lines).
- Adjust ElevenLabs voice settings (stability/similarity).
- Add content-type + filename headers if you want the endpoint to return an MP3 file directly.
*/
